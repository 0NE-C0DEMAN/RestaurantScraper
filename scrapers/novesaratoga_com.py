"""
Scraper for novesaratoga.com (Nové Italian Restaurant)
Scrapes menu from PDF files: In House Menu, Cocktail Menu, and Takeout Menu
...
MENU_PDFS = [
    {
        "url": "https://www.novesaratoga.com/wp-content/uploads/2025/05/NOVE-IN-HOUSE-MENU-2025.pdf",
        "menu_name": "In House Menu",
        "menu_type": "In House"
    },
    {
        "url": "https://www.novesaratoga.com/wp-content/uploads/2024/07/Nove-Cocktail-Menu.pdf",
        "menu_name": "Cocktail Menu",
        "menu_type": "Cocktails"
    },
    {
        "url": "https://www.novesaratoga.com/wp-content/uploads/2025/05/NOVE-TAKEOUT-MENU-SINGLE-2025.pdf",
        "menu_name": "Takeout Menu",
        "menu_type": "Takeout"
    }
]
...
4. **section**: The section/category name (e.g., "Appetizers", "Entrees", "Pasta", "Pizza", "Cocktails", "Wine", etc.)
...
- Look carefully at the menu - size labels are often shown near the prices
...
[
  ...
  {{
    ...
    "section": "Salads"
        ...
        # Process pages in pairs for efficiency (like we did for Mazzone)
...
def format_price(price: str) -> str:
    ...
    if '|' in price:
        ...
        for i, part in enumerate(price_parts):
            ...
            size_match = re.search(r'\b(small|large|regular|family|single|double|half|full|petite|grande)\b', part.lower())
            if size_match:
...
def scrape_nove_menu() -> List[Dict]:
    """Scrape menu from Nové Italian Restaurant"""
    print("=" * 60)
    print("Scraping Nové Italian Restaurant (novesaratoga.com)")
    print("=" * 60)
    
    restaurant_name = "Nové Italian Restaurant"
    restaurant_url = "http://www.novesaratoga.com/"
    ...
    for menu_info in MENU_PDFS:
        ...
        # Extract menu items
        print(f"    Extracting menu items...", end=" ")
        items = extract_menu_from_pdf_with_gemini(pdf_path, menu_name, menu_type)
        print(f"[OK] Found {len(items)} items")
        
        # Format and add restaurant info to each item
        for item in items:
            # Skip items that are just addons without description (they should be in descriptions of main items)
            name = item.get('name', '').strip().upper()
            if name.startswith('ADD ') and not item.get('description'):
                # This is likely an addon item - skip it as it should be in the description of the main item
                continue
            ...
            if price:
                ...
                # Skip if price is just "$" (empty price)
                if price == '$' or not price or price.strip() == '$':
                    price = ""
                item['price'] = price
            
            # Skip if no price and no description
            if not item.get('price') and not item.get('description'):
                continue
...
if __name__ == "__main__":
    items = scrape_nove_menu()
    
    # Save to JSON
    if items:
        output_dir = Path(__file__).parent.parent / "output"
        output_dir.mkdir(exist_ok=True)
        output_file = output_dir / "novesaratoga_com.json"
